...
void APP_BLE_Init(void)
{
   ...
   */
    ...
    /* FREERTOS_MARK */
    ADV_LP_REQ_Thread_Sem = osSemaphoreNew(1, 0, &ADV_LP_REQ_Thread_Sem_Attr);
    ADV_LP_REQ_Thread = osThreadNew(APP_BLE_AdvLowPower_Entry, NULL, &ADV_LP_REQ_Thread_Attr);
...
static void APP_BLE_AdvLowPower_timCB(void *arg)
{
   ...
   */
  /* FREERTOS_MARK */
  osSemaphoreRelease(ADV_LP_REQ_Thread_Sem);
...
void APP_BLE_AdvLowPower_Entry(void* thread_input)
{
  ...
  while(1)
  {
    osSemaphoreAcquire(ADV_LP_REQ_Thread_Sem, osWaitForever);
    /* FREERTOS_MARK */
    osMutexAcquire(LINK_LAYER_Thread_Mutex, osWaitForever);
    APP_BLE_AdvLowPower();
    /* FREERTOS_MARK */
